...
Pca9685Core::Pca9685Core(i2c_inst_t *aI2c, uint8_t aSda, uint8_t aScl,
                         uint8_t aI2cAdd) {
  mI2c = aI2c;
  mSda = aSda;
  mScl = aScl;
...
void Pca9685Core::SetPwm(uint16_t aChannel, uint16_t aOn, uint16_t aOff) {
  ...
  */
  ...
  // Serial.println("Channel " + String(aChannel) + " LED_ON " + String(aOn) + "
  // LED_OFF " + String(aOff));
  //  TODO: Add Debug statements
}

void Pca9685Core::SetServoPulse(uint16_t aChannel, uint16_t aPulse) {
  unsigned int pulse = (aPulse / 4095) * 4095;
